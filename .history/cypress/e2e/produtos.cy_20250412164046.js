...
describe('Teste da funcionalidade produto', () => {
    ...
    it('Cadastrar produtos', () => {
        ...
        cy.cadastrarProduto(token, produto, 220, "produto", 20)
            .then((response) => {
                ...
                nomeProduto = response.body.nome
            })
    });
    ...
    it('Editar produto', () => {
        cy.request({
            ...
            expected(response.body.message).to.equal("Registro alterado com sucesso")
        })
    });
    ...
    it('Buscar por produto', () => {
        cy.request({
            ...
            expected(response.status).to.equal(200)
            expected(response.body.nome).to.equal(nomeProduto)
        })
    });
    ...
    it('Excluir Produto', () => {
        cy.request({
            ...
            expected(response.status).to.equal(200)
            expected(response.body.message).to.equal("Registro excluído com sucesso")
        })
    });
